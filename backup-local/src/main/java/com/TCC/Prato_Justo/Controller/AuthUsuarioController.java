...
public class AuthUsuarioController {
   ...
   private final AuthService authService;
    ...
    public ResponseEntity<?> registro(@RequestBody CadastroRequest request){
        try {
            // Impede que estabelecimentos/ONG usem o endpoint genérico de usuário
            if (request.getPerfil() != null &&
                "ESTABELECIMENTO_ONG".equalsIgnoreCase(request.getPerfil())) {
                return ResponseEntity.status(HttpStatus.BAD_REQUEST)
                        .body("Para estabelecimentos, use o endpoint /auth/estabelecimento.");
            }
